...
- **2026-09-21 (Hermes session): STAGE 2 COMPLETE — all five AI-gate tasks landed, delegated one-by-one to TDD specialists and verified by the parent before push.**
  - **Task 7 (grounding review gate) — `e261fc9`.** New `applyGroundingGate()` in extraction/run.ts: a critical field (payableAmount, taxAmount, supplierName, supplierAbn, issueDate — the real intersection of validators' CRITICAL_FIELDS and shadow's fieldsToGround; the plan's gstAmount/taxCode names don't exist in this codebase) with no supporting span is needs_review even at 0.99 confidence; a MISSING report (legacy rows) is fail-closed needs_review; a report measured against a DIFFERENT value than this run produced counts as ungrounded. Stored report is READ, never re-run. Honest cost, stated in the worker comment: a first extraction has no report yet, so fresh captures go to review until the shadow stage has measured their pages. 195 tests green after the parent fixed the subagent's two type errors (it never ran tsc — noted, pattern to watch).
  - **Task 11 (licence floor in production) — `514648d`.** `assertSidecarLicenceFloor` cross-checks the sidecar /health's `WEIGHTS_LICENCE` through the same `violatesLicenceFloor` as the registry; fails CLOSED on unrecognised licence strings; shadow stage refuses to run a violating engine; preflight makes it a fatal boot failure (not production-gated — a non-free engine has no right side to run on). Subagent verified the tax-engine README's "217 tests" claim is TRUE (test.each expansion defeats grep-based counting) — corrected nothing, finding recorded.
  - **Task 10 (cost caps) — `7ae1d01`.** `MAX_STATEMENT_PAGES = 40`, typed `StatementPageCapError` thrown before any DB read or model call (mapped 413 by errors.filter; the worker records failure and completes the job — no infinite backoff on a permanent structural refusal). Per-tenant daily extraction budget `EXTRACTION_DAILY_BUDGET` (default 200, staging guess noted in its comment) enforced at all three enqueue points; 429 `quota_exhausted` on fresh work, `quotaExhausted: true` on duplicates during exhaustion.
  - **Task 9 (statement classification by its own bytes) — `40be660`.** Mostly already landed 2026-09-18 (`6329cc7`, Lane T); this task pinned the whole chain with a real e2e-style test: 3-page PDF → storage `original_storage_key` → worker reads the bytes → classification from the PDF's native text layer returns 'statement', not the 'receipt' fallback.
  - **Task 12 (BigInt pre-posting money) — `5ed9fba`.** New exact `linesGap()` in tax-rules (BigInt, mirroring the numeric rule); transactions.repo.ts sums via SQL `::text` and compares against '0.02' exactly; run.ts accumulates gstFreeTotal/lineSum with tax-rules `add()`. Float edge cases that wrongly PASSED before (0.03−0.01 landing at 0.01999… in IEEE) now refused. Full workspace green: tax-engine 217 golden, server 752, db 295, web 101, docai 48, mobile 141, tax-rules 92.
  - **Stage 2 outcome:** the AI gate is now real — grounding enforced, licence checked by the thing that runs the model, cost-capped at page and budget level, statements classified by their own bytes, and the gap check counts in the same BigInt as the ledger. All pushed to main (`5ed9fba`); CI/deploy verification runs via the poller as usual.
