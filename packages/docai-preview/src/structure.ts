...
const TOTAL_WORDS = /\b(TOTAL|AMOUNT\s*DUE|BALANCE\s*DUE|EFTPOS|VISA|MASTERCARD|AMEX|CASH|TO\s*PAY)\b/i;
...
function payableAmount(lines: PositionedLine[]): PreviewField {
  const candidates: Array<{ spans: Span[]; text: string; page: number; y: number }> = [];
  for (const { line, page, text } of lines) {
    if (!TOTAL_WORDS.test(text) || NOT_TOTAL.test(text)) continue;
    for (const run of moneyRuns(line)) {
      candidates.push({ ...run, page, y: line.box?.y ?? 0 });
    }
  }
...
function supplier(lines: PositionedLine[]): PreviewField {
  ...
  // Top fifth of the page only: a supplier name is printed at the head of a
  // docket, and the largest type further down is usually the total.
  const maxY = Math.max(...withHeight.map((l) => (l.line.box?.y ?? 0)));
  const top = withHeight.filter((l) => (l.line.box?.y ?? 0) <= maxY * 0.2);
